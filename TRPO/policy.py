...
class NN(object):
    ...
    def __init__(self, s_dim, a_dim):
        ...
        self.model.log_std = torch.nn.Parameter(torch.ones(self.a_dim, requires_grad=True)*1.5)
    ...
    def get_gradients_as_tensor(self, requires_grad=False):
        parameters = list(self.model.parameters())
        number_cols = sum(p.numel() for p in self.model.parameters())
        gradient = torch.zeros(self.a_dim, number_cols, requires_grad=requires_grad)
